...
var _ = Describe("CosmosDB Query", func() {
	...
	It("should not have write access on the read-only deployment",
		labels.RequireNothing,
		labels.High,
		labels.Positive,
		labels.CoreInfraService,
		...
		func(ctx context.Context) {
			...
			By("calling the write-check endpoint to verify RBAC denies write access")
			resp, statusCode, err := tc.CosmosWriteCheck(ctx, currentIdentity)
			Expect(err).NotTo(HaveOccurred(), "write-check request failed")
			Expect(statusCode).To(Equal(http.StatusOK), "expected 200 OK from write-check endpoint")
			Expect(resp.WriteAllowed).To(BeFalse(), "read-only deployment should not have write access to CosmosDB")
		},
	)
})
