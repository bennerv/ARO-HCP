...
Velero is installed using the CLI from the oadp-1.5-latest image. The CLI is used instead of the upstream Helm chart because this installation method is vetted by the OADP QA team.
...
The Velero CLI does not provide an install flag to add tolerations or node scheduling to pods.
The Velero CLI has a bug where it does not truncate init container names to be less than 63 characters.  See https://github.com/vmware-tanzu/velero/issues/9444.
...
4. The Job's main container truncates the init container name to be less than 63 characters, a temporary work around for https://github.com/vmware-tanzu/velero/issues/9444.
