...
func TestVersionSlug(t *testing.T) {
	slug, err := Branch{Name: "release-4.22"}.VersionSlug()
	require.NoError(t, err)
	assert.Equal(t, "4-22", slug)

	slug, err = Branch{Name: "release-5.0"}.VersionSlug()
	require.NoError(t, err)
	assert.Equal(t, "5-0", slug)

	_, err = Branch{Name: "main"}.VersionSlug()
	assert.Error(t, err)
}
