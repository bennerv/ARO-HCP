...
// VersionSlug returns the OCP major and minor version separated by a hyphen,
// e.g. "release-4.22" -> "4-22". This is used in marketplace SKU names.
func (b Branch) VersionSlug() (string, error) {
	m := branchNameRE.FindStringSubmatch(b.Name)
	if m == nil {
		return "", fmt.Errorf("branch name %q does not match release-<major>.<minor>", b.Name)
	}
