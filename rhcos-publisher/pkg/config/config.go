...
type Branch struct {
	// Name is the openshift/installer branch name, e.g. "release-5.0".
	Name string `yaml:"name"`
	// RHELVersions selects the coreos stream files to poll, e.g. [9, 10]
	// polls coreos-rhel-9.json and coreos-rhel-10.json.
	RHELVersions []int `yaml:"rhelVersions"`
...
func (c *Config) Validate() error {
	...
	for i, branch := range c.Branches {
		...
		if len(branch.RHELVersions) == 0 {
			errs = append(errs, fmt.Errorf("branches[%d]: at least one rhelVersion is required", i))
		}
		for _, v := range branch.RHELVersions {
			if v <= 0 {
				errs = append(errs, fmt.Errorf("branches[%d]: rhelVersions must be positive", i))
				break
			}
		}
	}
...
// Version returns the OCP version parsed from the branch name,
// e.g. "release-4.22" -> semver.Version{Major: 4, Minor: 22}.
func (b Branch) Version() (semver.Version, error) {
	m := branchNameRE.FindStringSubmatch(b.Name)
	if m == nil {
		return semver.Version{}, fmt.Errorf("branch name %q does not match release-<major>.<minor>", b.Name)
	}
	return semver.Parse(m[1] + "." + m[2] + ".0")
}
