...
// FetchVHD returns the path of a verified .vhd file for the image. If a
// previous attempt left a valid VHD on disk (SHA256 matches), it is reused
// without re-downloading. Otherwise the compressed VHD is downloaded,
// verified, decompressed, and re-verified. On error, all temporary files are
// cleaned up. The caller is responsible for deleting the returned file when
...
func (d *Downloader) FetchVHD(ctx context.Context, image stream.Image) (_ string, err error) {
	...
	if hash, hashErr := fileHash(vhdPath); hashErr == nil {
		if hash == image.UncompressedSHA256 {
			return vhdPath, nil
		}
		_ = os.Remove(vhdPath)
	}
