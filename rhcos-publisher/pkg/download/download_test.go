...
func TestFetchVHDReusesExistingValid(t *testing.T) {
	content := []byte("pretend this is a VHD")
	_, _, uncompressedSHA := gzipPayload(t, content)

	workDir := t.TempDir()
	vhdPath := filepath.Join(workDir, "rhcos-9.8.20260520-0-azure.x86_64.vhd")
	require.NoError(t, os.WriteFile(vhdPath, content, 0o600))

	downloader := New(workDir)
	result, err := downloader.FetchVHD(t.Context(), stream.Image{
		Branch:             "release-5.0",
		Arch:               "x86_64",
		Release:            "9.8.20260520-0",
		DownloadURL:        "http://should-not-be-called",
		CompressedSHA256:   "irrelevant",
		UncompressedSHA256: uncompressedSHA,
	})
	require.NoError(t, err)
	assert.Equal(t, vhdPath, result)
}

func TestFetchVHDDeletesExistingInvalid(t *testing.T) {
	content := []byte("pretend this is a VHD")
	compressed, compressedSHA, uncompressedSHA := gzipPayload(t, content)
	...
	workDir := t.TempDir()
	vhdPath := filepath.Join(workDir, "rhcos-9.8.20260520-0-azure.x86_64.vhd")
	require.NoError(t, os.WriteFile(vhdPath, []byte("corrupt data"), 0o600))
