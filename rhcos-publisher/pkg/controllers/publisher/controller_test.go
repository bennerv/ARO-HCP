...
func (f *fakeStatusClient) Get(_ context.Context, branch string, rhelVersion int) (*v1alpha1.RHCOSRelease, error) {
	key := fmt.Sprintf("%s/rhel%d", branch, rhelVersion)
...
func newFixture(existingPlans map[string]string) (*Syncer, *fakeIngestion, *fakeStatusClient, *fakeStorage) {
	...
	statusClient := &fakeStatusClient{statuses: map[string]*v1alpha1.RHCOSReleaseStatus{
		fmt.Sprintf("%s/rhel%d", testBranch, testRHELVersion): {},
	}}
	storageClient := &fakeStorage{blobs: map[string]bool{
		storage.BlobPath(testBranch, testRHELVersion, testArch, testRelease): true,
	}}
	cfg := &config.Config{
	...
	installerCache := &fakeInstallerCache{images: map[string]stream.Image{
		testKey: {Branch: testBranch, RHELVersion: testRHELVersion, Arch: testArch, Release: testRelease},
	}}
...
func TestSyncOnceCreatesPlansAndDraft(t *testing.T) {
	...
	// Two configure calls: plan creation, then tech config.
	require.Len(t, ingestion.configureLog, 2)
	assert.Len(t, ingestion.configureLog[0], 1, "x86 Gen2 plan created")
	assert.Len(t, ingestion.configureLog[1], 1, "plan gets a tech config")

	statusKey := fmt.Sprintf("%s/rhel%d", testBranch, testRHELVersion)
	archStatus := statusClient.statuses[statusKey].Architectures[testArch]
...
func TestSyncOnceExistingPlansOnlyUpdatesTechConfig(t *testing.T) {
	...
	require.Len(t, techConfigs, 1)
	versions := techConfigs[0]["vmImageVersions"].([]any)
	require.Len(t, versions, 1)
	assert.Equal(t, "50.98.20260520", versions[0].(map[string]any)["versionNumber"])
}

func TestSyncOnceDraftAlreadyConfiguredIsNoOp(t *testing.T) {
	syncer, ingestion, statusClient, _ := newFixture(map[string]string{})
	statusKey := fmt.Sprintf("%s/rhel%d", testBranch, testRHELVersion)
	statusClient.statuses[statusKey].Architectures = map[string]v1alpha1.RHCOSReleaseArchStatus{
