...
func TestSyncOnceCachesVersionsAndEnqueuesChanges(t *testing.T) {
	versionLister := &fakeVersionLister{versions: map[string][]string{
		"aro_4-22_x64_gen2": {"422.98.20260101"},
		"aro_4-22_arm_gen2": {},
	}}
	...
	assert.True(t, syncer.HasVersion("aro_4-22_x64_gen2", "422.98.20260101"))
	assert.False(t, syncer.HasVersion("aro_4-22_arm_gen2", "422.98.20260101"))
	...
	versionLister.versions["aro_4-22_arm_gen2"] = []string{"422.98.20260101"}
	require.NoError(t, syncer.SyncOnce(t.Context(), SyncKey))
	assert.Equal(t, []string{"release-4.22/aarch64"}, enqueued)
	assert.True(t, syncer.HasVersion("aro_4-22_arm_gen2", "422.98.20260101"))
}

func TestSyncOnceListError(t *testing.T) {
	versionLister := &fakeVersionLister{
		versions: map[string][]string{},
		errs:     map[string]error{"aro_4-22_x64_gen2": fmt.Errorf("throttled")},
	}
	statusClient := &fakeStatusClient{}
