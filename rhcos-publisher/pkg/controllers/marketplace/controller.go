...
func (s *Syncer) SyncOnce(ctx context.Context, _ string) error {
	...
	for _, branch := range s.cfg.Branches {
		...
		for _, arch := range []string{config.ArchX86_64, config.ArchAarch64} {
			...
			if changed {
				for _, rv := range branch.RHELVersions {
					key := fmt.Sprintf("%s/rhel%d/%s", branch.Name, rv, arch)
		...
		if branchSucceeded {
			now := metav1.Now()
			for _, rv := range branch.RHELVersions {
				if err := s.statusClient.UpdateStatus(ctx, branch.Name, rv, func(releaseStatus *v1alpha1.RHCOSReleaseStatus) {
					releaseStatus.LastMarketplaceSync = &now
				}); err != nil {
					errs = append(errs, fmt.Errorf("branch %s/rhel%d: %w", branch.Name, rv, err))
				}
			}
		}
	}
