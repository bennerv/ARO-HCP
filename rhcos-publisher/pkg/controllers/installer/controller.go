...
type StatusClient interface {
	UpdateStatus(ctx context.Context, branch string, rhelVersion int, mutate func(*v1alpha1.RHCOSReleaseStatus)) error
}
...
func (s *Syncer) SyncOnce(ctx context.Context, _ string) error {
	...
	for _, branch := range s.cfg.Branches {
		for _, rhelVersion := range branch.RHELVersions {
			images, err := s.streamClient.FetchImages(ctx, branch.Name, rhelVersion)
			if err != nil {
				allSucceeded = false
				errs = append(errs, fmt.Errorf("branch %s/rhel%d: %w", branch.Name, rhelVersion, err))
				continue
			}

			for _, changedKey := range s.updateBranchCache(branch.Name, rhelVersion, images) {
				logger.Info("installer image changed", "key", changedKey)
				s.enqueueReconcile(ctx, changedKey)
			}

			now := metav1.Now()
			if err := s.statusClient.UpdateStatus(ctx, branch.Name, rhelVersion, func(releaseStatus *v1alpha1.RHCOSReleaseStatus) {
...
// updateBranchCache replaces the cached images of one (branch, rhelVersion)
// pair and returns the keys whose image was added, changed or removed.
func (s *Syncer) updateBranchCache(branchName string, rhelVersion int, images []stream.Image) []string {
	s.mu.Lock()
	defer s.mu.Unlock()
	...
	var changed []string
	prefix := fmt.Sprintf("%s/rhel%d/", branchName, rhelVersion)
