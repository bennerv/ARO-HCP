...
func BlobPath(branch string, rhelVersion int, arch, release string) string {
	return fmt.Sprintf("rhcos/%s/rhel%d/%s/rhcos-%s-azure.%s.vhd", branch, rhelVersion, arch, release, arch)
}

// BlobPrefix returns the prefix for all blobs of a branch/rhelVersion/arch
// triple.
func BlobPrefix(branch string, rhelVersion int, arch string) string {
	return fmt.Sprintf("rhcos/%s/rhel%d/%s/", branch, rhelVersion, arch)
}
