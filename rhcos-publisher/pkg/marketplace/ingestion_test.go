  ...
  "resources": [
    {
      ...
      "id": "plan/aaa/bbb",
      "identity": {"externalId": "aro_50-x64"},
      "alias": "aro_50-x64"
    },
    {
      ...
      "skus": [
        {"skuId": "aro_50-x64", "imageType": "x64Gen2", "securityType": ["trusted"]}
      ],
      "vmImageVersions": [
        {"versionNumber": "50.98.20260101", "vmImages": [{"imageType": "x64Gen2", "source": {"sourceType": "sasUri", "osDisk": {"uri": "https://old"}}}]}
      ]
    }
  ]
}`
...
func TestFindPlanDurableID(t *testing.T) {
	tree := testTree(t)
	planID, found := FindPlanDurableID(tree, "aro_50-x64")
	require.True(t, found)
	assert.Equal(t, "plan/aaa/bbb", planID)

	_, found = FindPlanDurableID(tree, "aro_51-x64")
...
func TestEnsureSKUs(t *testing.T) {
	...
	changed := EnsureSKUs(techConfig, []SKU{{Name: "aro_50-x64", ImageType: ImageTypeX64Gen2, SecurityTypes: []string{"trusted"}}})
	assert.False(t, changed)

	// New SKU appended, existing entries untouched.
	changed = EnsureSKUs(techConfig, []SKU{{Name: "aro_50-arm", ImageType: ImageTypeARM64Gen2, SecurityTypes: []string{"trusted"}}})
	assert.True(t, changed)
	skus := techConfig["skus"].([]any)
	require.Len(t, skus, 2)
	assert.Equal(t, "aro_50-arm", skus[1].(map[string]any)["skuId"])
}
...
func TestEnsureImageVersion(t *testing.T) {
	...
	changed := EnsureImageVersion(techConfig, "50.98.20260101", []VMImage{{ImageType: ImageTypeX64Gen2, URI: "https://new"}})
	assert.False(t, changed)

	changed = EnsureImageVersion(techConfig, "50.98.20260520", []VMImage{{ImageType: ImageTypeX64Gen2, URI: "https://web.example.com/x.vhd"}})
	assert.True(t, changed)

	versions := techConfig["vmImageVersions"].([]any)
	require.Len(t, versions, 2)
	added := versions[1].(map[string]any)
	assert.Equal(t, "50.98.20260520", added["versionNumber"])
...
func TestNewPlanResource(t *testing.T) {
	plan := NewPlanResource("product/aaa", "aro_51-x64", "aro_51-x64")
	assert.Equal(t, "product/aaa", plan["product"])
	assert.Equal(t, "aro_51-x64", plan["identity"].(map[string]any)["externalId"])
