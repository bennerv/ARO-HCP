  ...
  "resources": [
    {
      ...
      "id": "plan/aaa/bbb",
      "identity": {"externalId": "aro_4-22_x64_gen2"},
      "alias": "aro_4-22_x64_gen2"
    },
    {
      ...
      "skus": [
        {"skuId": "aro_4-22_x64_gen2", "imageType": "x64Gen2", "securityType": ["trusted"]}
      ],
      "vmImageVersions": [
...
func TestFindPlanDurableID(t *testing.T) {
	tree := testTree(t)
	planID, found := FindPlanDurableID(tree, "aro_4-22_x64_gen2")
	require.True(t, found)
	assert.Equal(t, "plan/aaa/bbb", planID)

	_, found = FindPlanDurableID(tree, "aro_4-23_x64_gen2")
...
func TestEnsureSKUs(t *testing.T) {
	...
	changed := EnsureSKUs(techConfig, []SKU{{Name: "aro_4-22_x64_gen2", ImageType: ImageTypeX64Gen2, SecurityTypes: []string{"trusted"}}})
	assert.False(t, changed)

	// New SKU appended, existing entries untouched.
	changed = EnsureSKUs(techConfig, []SKU{{Name: "aro_4-22_arm_gen2", ImageType: ImageTypeARM64Gen2, SecurityTypes: []string{"trusted"}}})
	assert.True(t, changed)
	skus := techConfig["skus"].([]any)
	require.Len(t, skus, 2)
	assert.Equal(t, "aro_4-22_arm_gen2", skus[1].(map[string]any)["skuId"])
}
...
func TestNewPlanResource(t *testing.T) {
	plan := NewPlanResource("product/aaa", "aro_4-23_x64_gen2", "aro_4-23_x64_gen2")
	assert.Equal(t, "product/aaa", plan["product"])
	assert.Equal(t, "aro_4-23_x64_gen2", plan["identity"].(map[string]any)["externalId"])
