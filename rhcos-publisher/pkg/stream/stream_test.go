...
func TestFetchImages(t *testing.T) {
	...
	images, err := client.FetchImages(t.Context(), "release-5.0", 9)
	require.NoError(t, err)
	assert.Equal(t, "/release-5.0/data/data/coreos/coreos-rhel-9.json", requestedPath)
	...
	assert.Equal(t, "release-5.0/rhel9/x86_64", byArch["x86_64"].Key())
...
func TestFetchImagesHTTPError(t *testing.T) {
	...
	client := NewClient(server.URL)
	_, err := client.FetchImages(t.Context(), "release-5.99", 9)
	assert.Error(t, err)
}
...
func TestFetchImagesIncompleteArtifact(t *testing.T) {
  ...
  "architectures": {
	...
	client := NewClient(server.URL)
	_, err := client.FetchImages(t.Context(), "release-5.0", 9)
	require.Error(t, err)
