...
// Image describes one downloadable RHCOS Azure VHD of a
// branch/rhelVersion/architecture triple, as pinned by the branch's coreos
// stream file.
type Image struct {
	// Branch is the openshift/installer branch, e.g. "release-5.0".
	Branch string
	// RHELVersion is the major RHEL version of the coreos stream, e.g. 9.
	RHELVersion int
...
// Key is the workqueue key of the image's branch/rhelVersion/architecture
// triple, e.g. "release-5.0/rhel9/x86_64".
func (i Image) Key() string {
	return fmt.Sprintf("%s/rhel%d/%s", i.Branch, i.RHELVersion, i.Arch)
}
